import { PROMOTION_STATUS, PROMOTION_STATUS_LABELS } from "../constants";
...
const statusClasses = {
  ...
  [PROMOTION_STATUS.DRAFT]: "bg-slate-50 text-slate-700 ring-slate-200",
};

export default function PromotionStatusBadge({ status }) {
  const label = PROMOTION_STATUS_LABELS[status] || status || "Unknown";

  return (
    <span
      aria-label={`Promotion status: ${label}`}
