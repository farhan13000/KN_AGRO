...
export default function SalesManagerQuotationEditPage() {
  ...
  const quotationActions = useQuotationActions({
    onSuccess: () => navigate(`${ROUTES.SALES_MANAGER.QUOTATIONS}/${quotationId}`),
  });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      // quotation left DRAFT between load and submit, refetching re-syncs
      // `quotation.status` so the DRAFT-only gate above correctly swaps in
      // the "not editable" block instead of leaving a stale form on screen.
  ...
  if (!quotation) {
    return (
      <EmptyState
        ...
        actionTo={ROUTES.SALES_MANAGER.QUOTATIONS}
  ...
  if (quotation.status !== QUOTATION_STATUS.DRAFT) {
    return (
      <EmptyState
        ...
        actionTo={`${ROUTES.SALES_MANAGER.QUOTATIONS}/${quotationId}`}
